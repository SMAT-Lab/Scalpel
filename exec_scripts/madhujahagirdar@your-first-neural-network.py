...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            targets_list = np.array(y, ndmin=2).T
            
            
            hidden_inputs = np.dot(self.weights_input_to_hidden,inputs)
            # signals from hidden layer = 16x128
            ...
            error = targets_list - final_outputs # Output layer error is the difference between desired target and actual output.
            ...
            # Weight step (input to hidden) 16x128
            # Weight step (hidden to output)
            inputs_transpose = inputs.T
            delta_weights_i_h += np.dot(hidden_error_term,inputs_transpose)
...
print((test_targets['cnt']*std + mean).shape)
