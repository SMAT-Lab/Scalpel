...
# Removed the test data from the data set 
data = data[:-21*24]
...
class NeuralNetwork(object):
    ...
    def run(self, features):
        ...
        #### Implement the forward pass here ####
        # Hidden layer - replaced these values with the appropriate calculations.
        hidden_inputs = np.dot(features, self.weights_input_to_hidden) # signals into hidden layer
        hidden_outputs = 1 / (1 + np.exp(-hidden_inputs)) # signals from hidden layer
        
        # Output layer - Replaced these values with the appropriate calculations.
        final_inputs = np.dot(hidden_outputs,self.weights_hidden_to_output) # signals into final output layer
        final_outputs = final_inputs # 1 / (1 + np.exp(-final_inputs)) # signals from final output layer 
...
# In[12]:
# Print the shape of the inputs and outputs to better understand the structure of the network
...
iterations = 3000
learning_rate = 0.5
hidden_nodes = 15  # I have chosen a small number of Hidden nodes becuase it performed best
output_nodes = 1
