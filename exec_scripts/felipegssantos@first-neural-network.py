...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_error = np.dot(self.weights_hidden_to_output, error) # this is right only because f(h)=h
            
            # TODO: Backpropagated error terms - Replace these values with your calculations.
            output_error_term = error # f(h)=h => f'(h)=1
            hidden_error_term = hidden_error * hidden_outputs * (1-hidden_outputs) # f(x)=sigmoid(x) => f'(x)=f(x)(1-f(x))
