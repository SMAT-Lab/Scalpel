...
model = Sequential()
# Include LSTM with 5 hidden nodes, the input shape input_shape=(window_size,1)))
...
#print(sample_text)
# remove all execept characters, fullstop comma, question mark, exclamation from text
...
#Define sequential model
model = Sequential()
#Include LSTM with 200 hidden nodes, with input shape of window_size, len(chars))
model.add(LSTM(200, input_shape=(window_size, len(chars))))
#Include fully connected layer with hidden units of len(chars) or number of unique characters
model.add(Dense(len(chars)))
#Use softmax activation for solving multiclass classification based probability distribution 
