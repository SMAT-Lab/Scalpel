...
class Rewriter:
    ...
    def random_var_renaming(self, new_name_candidates=[], K=2):
        ...
        var_name_set = [var["name"]
                        for var in all_vars if "." not in var["name"]]
    ...
    def unused_stmt_insertion(self):
        inserted_stmt = ast.Expr(ast.Call(ast.Name("print", ast.Load()), [
    ...
    def for2while(self, node):
        if isinstance(node, ast.For) and isinstance(node.target, ast.Name):
            ...
            max_counter_var = ast.Name(
                "_len_of_iter_" + str(node.lineno), ast.Store())
            ...
            counter_inc_stmt = ast.AugAssign(
                counter_var, ast.Add(), ast.Constant(1))

            new_target_var = ast.Subscript(
                iter_object, counter_var, ast.Load())
...
class ASTRewriter(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        ...
        if len(node.targets) == 1 and isinstance(node.value, ast.ListComp):
            ...
            if len(ifs) == 0:
                append_attr = ast.Attribute(value=ast.Name(
                    id=new_lst_name, ctx=ast.Load()), attr='append', ctx=ast.Load())
